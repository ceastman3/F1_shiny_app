...
# results$position[is.na(results$position)] <- "DNF"
head(results)
```
...
server = function(input, output) {
  ...
  output$schedule = renderDataTable({
    races %>%
      select(round, date, race_name, circuit_name, location, country)
  })
   
  output$position_table = renderDataTable({
    results %>%
      filter(raceId == selected_raceId()) |>
      select(position, forename, surname, driver_num, nationality, time, points, status)
  })
}
